defmodule(SnappyData.Thrift.SnappyDataService) do
      ...
      def(serialize(%ForgetXATransactionArgs{conn_id: conn_id, xid: xid, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<12, 2::size(16)>> | SnappyData.Thrift.TransactionXid.serialize(xid)]
        end, case(token) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(GetClobChunkArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<10, 2::16-signed, value::size(64), rest::binary>>, acc)) do
      ...
      def(serialize(%GetClobChunkArgs{conn_id: conn_id, lob_id: lob_id, offset: offset, size: size, free_lob_at_end: free_lob_at_end, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            <<10, 2::size(16), lob_id::64-signed>>
        end, case(offset) do
          nil ->
            <<>>
          _ ->
            <<10, 3::size(16), offset::64-signed>>
        end, case(size) do
          nil ->
            <<>>
          _ ->
            <<8, 4::size(16), size::32-signed>>
      ...
      def(serialize(%StartXATransactionArgs{conn_id: conn_id, xid: xid, timeout_in_seconds: timeout_in_seconds, flags: flags, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            <<8, 3::size(16), timeout_in_seconds::32-signed>>
        end, case(flags) do
          nil ->
            <<>>
          _ ->
            <<8, 4::size(16), flags::32-signed>>
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%ExecutePreparedQueryArgs{stmt_id: stmt_id, params: params, attrs: attrs, token: token})) do
        [case(stmt_id) do
          nil ->
            <<>>
          _ ->
            <<10, 1::size(16), stmt_id::64-signed>>
        end, case(params) do
          nil ->
            <<>>
          _ ->
            [<<12, 2::size(16)>> | SnappyData.Thrift.Row.serialize(params)]
        end, case(attrs) do
          nil ->
            <<>>
          _ ->
            [<<12, 3::size(16)>> | SnappyData.Thrift.StatementAttrs.serialize(attrs)]
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%GetBestRowIdentifierArgs{metadata_args: metadata_args, scope: scope, nullable: nullable})) do
        [case(metadata_args) do
          nil ->
            <<>>
          _ ->
            [<<12, 1::size(16)>> | SnappyData.Thrift.ServiceMetaDataArgs.serialize(metadata_args)]
        end, case(scope) do
          nil ->
            <<>>
          _ ->
            <<8, 2::size(16), scope::32-signed>>
  ...
  defmodule(ExecutePreparedArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<13, 3::16-signed, 8, 12, map_size::size(32), rest::binary>>, struct)) do
      ...
      defp(deserialize__output_params__key(<<key::size(32), rest::binary>>, stack)) do
      ...
      def(serialize(%ExecutePreparedArgs{stmt_id: stmt_id, params: params, output_params: output_params, attrs: attrs, token: token})) do
        [case(stmt_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<13, 3::size(16), 8, 12, Enum.count(output_params)::size(32)>> | for({k, v} <- output_params) do
          ...
          nil ->
            <<>>
          _ ->
            [<<12, 4::size(16)>> | SnappyData.Thrift.StatementAttrs.serialize(attrs)]
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%PrepareAndExecuteArgs{conn_id: conn_id, sql: sql, params_batch: params_batch, output_params: output_params, attrs: attrs, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<11, 2::size(16), byte_size(sql)::size(32)>> | sql]
        end, case(params_batch) do
          nil ->
            <<>>
          _ ->
            [<<15, 3::size(16), 12, length(params_batch)::size(32)>> | for(e <- params_batch) do
          ...
          nil ->
            <<>>
          _ ->
            [<<13, 4::size(16), 8, 12, Enum.count(output_params)::size(32)>> | for({k, v} <- output_params) do
              [<<k::32-signed>> | SnappyData.Thrift.OutputParameter.serialize(v)]
            end]
        end, case(attrs) do
          nil ->
            <<>>
          _ ->
            [<<12, 5::size(16)>> | SnappyData.Thrift.StatementAttrs.serialize(attrs)]
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%GetServiceMetaDataArgs{conn_id: conn_id, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<11, 2::size(16), byte_size(token)::size(32)>> | token]
        end | <<0>>]
      end
    end
    def(serialize(struct)) do
  ...
  defmodule(GetUDTsArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize__types(<<element::size(32), rest::binary>>, [list, remaining | stack])) do
      ...
      def(serialize(%GetUDTsArgs{metadata_args: metadata_args, types: types})) do
        [case(metadata_args) do
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 2::size(16), 8, length(types)::size(32)>> | for(e <- types) do
              <<e::32-signed>>
            end]
        end | <<0>>]
      end
  ...
  defmodule(CommitTransactionArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<13, 3::16-signed, 8, 2, map_size::size(32), rest::binary>>, struct)) do
      ...
      defp(deserialize__flags__key(<<key::size(32), rest::binary>>, stack)) do
      ...
      def(serialize(%CommitTransactionArgs{conn_id: conn_id, start_new_transaction: start_new_transaction, flags: flags, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<13, 3::size(16), 8, 2, Enum.count(flags)::size(32)>> | for({k, v} <- flags) do
      ...
      def(serialize(%CloseResultSetArgs{cursor_id: cursor_id, token: token})) do
        [case(cursor_id) do
          nil ->
            <<>>
          _ ->
            <<10, 1::size(16), cursor_id::64-signed>>
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%GetSchemaMetaDataArgs{schema_call: schema_call, metadata_args: metadata_args})) do
        [case(schema_call) do
          nil ->
            <<>>
          _ ->
            <<8, 1::size(16), schema_call::32-signed>>
        end, case(metadata_args) do
          nil ->
            <<>>
          _ ->
            [<<12, 2::size(16)>> | SnappyData.Thrift.ServiceMetaDataArgs.serialize(metadata_args)]
        end | <<0>>]
      end
    end
    def(serialize(struct)) do
      ...
      def(serialize(%OpenConnectionArgs{arguments: arguments})) do
        [case(arguments) do
          nil ->
            <<>>
          _ ->
            [<<12, 1::size(16)>> | SnappyData.Thrift.OpenConnectionArgs.serialize(arguments)]
        end | <<0>>]
      end
    end
    def(serialize(struct)) do
      ...
      def(serialize(%ScrollCursorArgs{cursor_id: cursor_id, offset: offset, offset_is_absolute: offset_is_absolute, fetch_reverse: fetch_reverse, fetch_size: fetch_size, token: token})) do
        [case(cursor_id) do
          ...
          nil ->
            <<>>
          _ ->
            <<8, 5::size(16), fetch_size::32-signed>>
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%BulkCloseArgs{entities: entities})) do
        [case(entities) do
          nil ->
            <<>>
          _ ->
            [<<15, 1::size(16), 12, length(entities)::size(32)>> | for(e <- entities) do
      ...
      def(serialize(%SendClobChunkArgs{chunk: chunk, conn_id: conn_id, token: token})) do
        [case(chunk) do
          nil ->
            <<>>
          _ ->
            [<<12, 1::size(16)>> | SnappyData.Thrift.ClobChunk.serialize(chunk)]
        end, case(conn_id) do
          nil ->
            <<>>
          _ ->
            <<10, 2::size(16), conn_id::64-signed>>
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%BeginTransactionArgs{conn_id: conn_id, isolation_level: isolation_level, flags: flags, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            <<3, 2::size(16), isolation_level::8-signed>>
        end, case(flags) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(ExecutePreparedBatchArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<15, 2::16-signed, 12, remaining::size(32), rest::binary>>, struct)) do
        deserialize__params_batch(rest, [[], remaining, struct])
      end
      defp(deserialize(<<12, 3::16-signed, rest::binary>>, acc)) do
        case(Elixir.SnappyData.Thrift.StatementAttrs.BinaryProtocol.deserialize(rest)) do
      ...
      def(serialize(%ExecutePreparedBatchArgs{stmt_id: stmt_id, params_batch: params_batch, attrs: attrs, token: token})) do
        [case(stmt_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 2::size(16), 12, length(params_batch)::size(32)>> | for(e <- params_batch) do
              SnappyData.Thrift.Row.serialize(e)
            end]
        end, case(attrs) do
          nil ->
  ...
  defmodule(SetTransactionAttributesArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<13, 2::16-signed, 8, 2, map_size::size(32), rest::binary>>, struct)) do
        deserialize__flags__key(rest, [%{}, map_size, struct])
      end
      defp(deserialize(<<11, 3::16-signed, string_size::32-signed, value::binary-size(string_size), rest::binary>>, acc)) do
        deserialize(rest, %{acc | token: value})
      ...
      def(serialize(%SetTransactionAttributesArgs{conn_id: conn_id, flags: flags, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<13, 2::size(16), 8, 2, Enum.count(flags)::size(32)>> | for({k, v} <- flags) do
              [<<k::32-signed>> | case(v) do
                nil ->
                  <<0>>
                false ->
      ...
      def(serialize(%SendBlobChunkArgs{chunk: chunk, conn_id: conn_id, token: token})) do
        [case(chunk) do
          nil ->
            <<>>
          _ ->
            [<<12, 1::size(16)>> | SnappyData.Thrift.BlobChunk.serialize(chunk)]
        end, case(conn_id) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(ExecuteCursorUpdateArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<15, 4::16-signed, 15, remaining::size(32), rest::binary>>, struct)) do
      ...
      defp(deserialize__changed_columns_list(<<8, inner_remaining::size(32), rest::binary>>, [list, remaining | stack])) do
      ...
      defp(deserialize__changed_columns_list__element(<<element::size(32), rest::binary>>, [list, remaining | stack])) do
      ...
      defp(deserialize__changed_row_indexes(<<element::size(32), rest::binary>>, [list, remaining | stack])) do
      ...
      defp(deserialize__operations(<<element::size(32), rest::binary>>, [list, remaining | stack])) do
      ...
      def(serialize(%ExecuteCursorUpdateArgs{cursor_id: cursor_id, operations: operations, changed_rows: changed_rows, changed_columns_list: changed_columns_list, changed_row_indexes: changed_row_indexes, token: token})) do
        [case(cursor_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 2::size(16), 8, length(operations)::size(32)>> | for(e <- operations) do
              <<e::32-signed>>
            end]
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 3::size(16), 12, length(changed_rows)::size(32)>> | for(e <- changed_rows) do
              SnappyData.Thrift.Row.serialize(e)
            end]
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 4::size(16), 15, length(changed_columns_list)::size(32)>> | for(e <- changed_columns_list) do
              [<<8, length(e)::size(32)>> | for(e <- e) do
                <<e::32-signed>>
              end]
            end]
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 5::size(16), 8, length(changed_row_indexes)::size(32)>> | for(e <- changed_row_indexes) do
              <<e::32-signed>>
            end]
        end, case(token) do
          nil ->
  ...
  defmodule(ExecuteUpdateArgs) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<15, 2::16-signed, 11, remaining::size(32), rest::binary>>, struct)) do
      ...
      def(serialize(%ExecuteUpdateArgs{conn_id: conn_id, sqls: sqls, attrs: attrs, token: token})) do
        [case(conn_id) do
          ...
          nil ->
            <<>>
          _ ->
            [<<15, 2::size(16), 11, length(sqls)::size(32)>> | for(e <- sqls) do
              [<<byte_size(e)::size(32)>> | e]
            end]
        end, case(attrs) do
          nil ->
      ...
      def(serialize(%GetNextResultSetArgs{cursor_id: cursor_id, other_result_set_behaviour: other_result_set_behaviour, token: token})) do
        [case(cursor_id) do
          ...
          nil ->
            <<>>
          _ ->
            <<3, 2::size(16), other_result_set_behaviour::8-signed>>
        end, case(token) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%GetClobChunkResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.ClobChunk.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%ExecutePreparedQueryResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.RowSet.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%ExecutePreparedResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.StatementResult.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%GetServiceMetaDataResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.ServiceMetaData.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
      ...
      def(serialize(%OpenConnectionResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.ConnectionProperties.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(GetTransactionAttributesResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<13, 0::16-signed, 8, 2, map_size::size(32), rest::binary>>, struct)) do
      ...
      defp(deserialize__success__key(<<key::size(32), rest::binary>>, stack)) do
      ...
      def(serialize(%GetTransactionAttributesResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<13, 0::size(16), 8, 2, Enum.count(success)::size(32)>> | for({k, v} <- success) do
              [<<k::32-signed>> | case(v) do
                nil ->
                  <<0>>
                false ->
  ...
  defmodule(SendClobChunkResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<10, 0::16-signed, value::size(64), rest::binary>>, acc)) do
        deserialize(rest, %{acc | success: value})
      end
      defp(deserialize(<<12, 1::16-signed, rest::binary>>, acc)) do
        case(Elixir.SnappyData.Thrift.SnappyException.BinaryProtocol.deserialize(rest)) do
      ...
      def(serialize(%SendClobChunkResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            <<10, 0::size(16), success::64-signed>>
        end, case(error) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(PrepareXATransactionResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<8, 0::16-signed, value::size(32), rest::binary>>, acc)) do
        deserialize(rest, %{acc | success: value})
      end
      defp(deserialize(<<12, 1::16-signed, rest::binary>>, acc)) do
        case(Elixir.SnappyData.Thrift.SnappyException.BinaryProtocol.deserialize(rest)) do
      ...
      def(serialize(%ExecutePreparedUpdateResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.UpdateResult.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(BeginTransactionResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<3, 0::16-signed, value, rest::binary>>, acc)) do
        deserialize(rest, %{acc | success: value})
      end
      defp(deserialize(<<12, 1::16-signed, rest::binary>>, acc)) do
        case(Elixir.SnappyData.Thrift.SnappyException.BinaryProtocol.deserialize(rest)) do
      ...
      def(serialize(%GetBlobChunkResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.BlobChunk.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
  ...
  defmodule(FetchActiveStatementsResponse) do
    ...
    defmodule(BinaryProtocol) do
      ...
      defp(deserialize(<<13, 0::16-signed, 10, 11, map_size::size(32), rest::binary>>, struct)) do
        deserialize__success__key(rest, [%{}, map_size, struct])
      end
      defp(deserialize(<<12, 1::16-signed, rest::binary>>, acc)) do
        case(Elixir.SnappyData.Thrift.SnappyException.BinaryProtocol.deserialize(rest)) do
      ...
      defp(deserialize__success__key(<<key::size(64), rest::binary>>, stack)) do
        deserialize__success__value(rest, key, stack)
      end
      defp(deserialize__success__key(_, _)) do
        :error
      ...
      def(serialize(%FetchActiveStatementsResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<13, 0::size(16), 10, 11, Enum.count(success)::size(32)>> | for({k, v} <- success) do
              [<<k::64-signed, byte_size(v)::size(32)>> | v]
            end]
        end, case(error) do
          nil ->
            <<>>
      ...
      def(serialize(%PrepareStatementResponse{success: success, error: error})) do
        [case(success) do
          nil ->
            <<>>
          _ ->
            [<<12, 0::size(16)>> | SnappyData.Thrift.PrepareResult.serialize(success)]
        end, case(error) do
          nil ->
            <<>>
          _ ->
