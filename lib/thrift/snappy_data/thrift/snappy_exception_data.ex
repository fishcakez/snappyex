defmodule(SnappyData.Thrift.SnappyExceptionData) do
  ...
  defmodule(BinaryProtocol) do
    ...
    defp(deserialize(<<15, 4::16-signed, 8, remaining::size(32), rest::binary>>, struct)) do
    ...
    defp(deserialize__update_counts(<<element::size(32), rest::binary>>, [list, remaining | stack])) do
    ...
    def(serialize(%SnappyData.Thrift.SnappyExceptionData{reason: reason, error_code: error_code, sql_state: sql_state, update_counts: update_counts})) do
      [case(reason) do
        ...
        _ ->
          [<<11, 1::size(16), byte_size(reason)::size(32)>> | reason]
        ...
        _ ->
          <<8, 2::size(16), error_code::32-signed>>
      end, case(sql_state) do
        nil ->
          <<>>
        _ ->
          [<<11, 3::size(16), byte_size(sql_state)::size(32)>> | sql_state]
      end, case(update_counts) do
        nil ->
          <<>>
        _ ->
          [<<15, 4::size(16), 8, length(update_counts)::size(32)>> | for(e <- update_counts) do
            <<e::32-signed>>
          end]
      end | <<0>>]
    end
